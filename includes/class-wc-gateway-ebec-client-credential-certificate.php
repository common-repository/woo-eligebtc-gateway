...
class WC_Gateway_EBEC_Client_Credential_Certificate extends WC_Gateway_EBEC_Client_Credential {
	...
	public function configure_curl( $handle, $r, $url ) {
		...
		if ( false === curl_setopt( $handle, CURLOPT_SSLCERT, $certificate_file ) ) {
			throw new Exception( __( 'Unable to accept certificate during cURL configuration', 'woocommerce-gateway-eligebtc' ), WC_Gateway_EBEC_Client::INVALID_ENVIRONMENT_ERROR );
		}

		if ( $this->_use_secure_transport() && false === curl_setopt( $handle, CURLOPT_SSLCERTPASSWD, $password ) ) {
			throw new Exception( __( 'Unable to accept certificate password during cURL configuration', 'woocommerce-gateway-eligebtc' ), WC_Gateway_EBEC_Client::INVALID_ENVIRONMENT_ERROR );
		}
	}
	...
	protected function _maybe_create_certificate_file( $password ) {
		...
		if ( ! $temp_file ) {
			throw new Exception( sprintf( __( 'Unable to write certificate file %s during cURL configuration', 'woocommerce-gateway-eligebtc' ), $temp_file ), WC_Gateway_EBEC_Client::INVALID_ENVIRONMENT_ERROR );
		}
	...
	protected function _maybe_create_secure_certificate_file( $temp_file, $password ) {
		$token = openssl_pkey_get_private( $this->_certificate );

		if ( false === $token ) {
			throw new Exception( __( 'Failed to retrieve token key during cURL configuration', 'woocommerce-gateway-eligebtc' ), WC_Gateway_EBEC_Client::INVALID_ENVIRONMENT_ERROR );
		}

		if ( ! openssl_pkcs12_export_to_file( $this->_certificate, $temp_file, $token, $password ) ) {
			throw new Exception( __( 'Failed to export PKCS12 file during cURL configuration', 'woocommerce-gateway-eligebtc' ), WC_Gateway_EBEC_Client::INVALID_ENVIRONMENT_ERROR );
		}
	}
