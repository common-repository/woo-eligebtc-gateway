...
/* Tiene una funcion que actualiza la configuracion */
...
/**
 * EligeBTC Express Integrated EligeBTC Signup Handler.
 */
class WC_Gateway_EBEC_IPS_Handler {
	...
	public function get_redirect_url( $env ) {
		...
		return add_query_arg(
			array(
				'env'                     => $env,
				'wc_ebec_ips_admin_nonce' => wp_create_nonce( 'wc_ebec_ips' ),
			),
	...
	protected function _redirect_with_messages( $error_msg ) {
		...
		add_option( 'woo_eb_admin_error', $error_msgs );
	...
	public function maybe_received_credentials() {
		...
		// Require the nonce.
		if ( empty( $_GET['wc_ebec_ips_admin_nonce'] ) || empty( $_GET['env'] ) ) {
			return false;
		}
		$env = in_array( $_GET['env'], array( 'live', 'sandbox' ) ) ? $_GET['env'] : 'live';

		// Verify the nonce.
		if ( ! wp_verify_nonce( $_GET['wc_ebec_ips_admin_nonce'], 'wc_ebec_ips' ) ) {
			wp_die( __( 'Invalid connection request', 'woocommerce-gateway-eligebtc' ) );
		}

		wc_gateway_EBEC_log( sprintf( '%s: returned back from IPS flow with parameters: %s', __METHOD__, print_r( $_GET, true ) ) );

		// Check if error.
		if ( ! empty( $_GET['error'] ) ) {
			$error_message = ! empty( $_GET['error_message'] ) ? $_GET['error_message'] : '';
			wc_gateway_EBEC_log( sprintf( '%s: returned back from IPS flow with error: %s', __METHOD__, $error_message ) );

			$this->_redirect_with_messages( __( 'Sorry, Easy Setup encountered an error.  Please try again.', 'woocommerce-gateway-eligebtc' ) );
		}

		// Make sure credentials present in query string.
		foreach ( array( 'api_style', 'api_username', 'api_password', 'signature' ) as $param ) {
			if ( empty( $_GET[ $param ] ) ) {
				wc_gateway_EBEC_log( sprintf( '%s: returned back from IPS flow but missing parameter %s', __METHOD__, $param ) );

				$this->_redirect_with_messages( __( 'Sorry, Easy Setup encountered an error.  Please try again.', 'woocommerce-gateway-eligebtc' ) );
			}
		}

		$creds = new WC_Gateway_EBEC_Client_Credential_Signature(
			$_GET['api_username'],
			$_GET['api_password'],
			$_GET['signature']
		);
		...
		$error_msgs = array();
		try {
			$payer_id = wc_gateway_ebec()->client->test_api_credentials( $creds, $env );

			if ( ! $payer_id ) {
				$this->_redirect_with_messages( __( 'Easy Setup was able to obtain your API credentials, but was unable to verify that they work correctly.  Please make sure your EligeBTC account is set up properly and try Easy Setup again.', 'woocommerce-gateway-eligebtc' ) );
			}
		} catch ( EligeBTC_API_Exception $ex ) {
			$error_msgs[] = array(
				'warning' => __( 'Easy Setup was able to obtain your API credentials, but an error occurred while trying to verify that they work correctly.  Please try Easy Setup again.', 'woocommerce-gateway-eligebtc' ),
			);
		}

		$error_msgs[] = array(
			'success' => __( 'Success!  Your EligeBTC account has been set up successfully.', 'woocommerce-gateway-eligebtc' ),
		);

		if ( ! empty( $error_msgs ) ) {
			wc_gateway_EBEC_log( sprintf( '%s: returned back from IPS flow: %s', __METHOD__, print_r( $error_msgs, true ) ) );
		}
