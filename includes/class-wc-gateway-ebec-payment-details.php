...
class EligeBTC_Payment_Details {
	public $token                   = false;
	...
	public function loadFromDoECResponse( $doECResponse ) {
		...
		if ( $max_payment_num >= 0 ) {
			...
			for ( $i = 0; $i <= $max_payment_num; $i++ ) {
				$this->payments[ $i ] = new EligeBTC_Payment_Payment_Details();
				$this->payments[ $i ]->loadFromDoECResponse( $doECResponse, $i );
			}
		}

		$this->shipping_option_details = new EligeBTC_Payment_Shipping_Option_Details();
...
class EligeBTC_Payment_Payment_FMF_Details {
		...
		foreach ( $max_filter_num as $index => $value ) {
			for ( $i = 0; $i <= $value; $i++ ) {
				...
				if ( array_key_exists( $prefix . 'NAME' . $i, $doECResponse ) && array_key_exists( $prefix . 'ID' . $i, $doECResponse ) ) {
					$filters[] = new EligeBTC_Payment_Fraud_Management_Filter( $doECResponse[ $prefix . 'NAME' . $i ], $doECResponse[ $prefix . 'ID' . $i ], $index );
				}
			}
		}
...
class EligeBTC_Payment_Fraud_Management_Filter {
	public $name;
	public $id;
	public $status;
...
class EligeBTC_Payment_Shipping_Option_Details {
...
class EligeBTC_Payment_Payment_Details {
	...
	public function loadFromDoECResponse( $doECResponse, $bucketNum ) {
		...
		$this->protection_eligibility_details = new EligeBTC_Payment_Payment_Protection_Eligibility_Details();
...
class EligeBTC_Payment_Payment_Protection_Eligibility_Details {
...
class EligeBTC_Payment_Payment_Instrument_Details {
	public $instrument_category          = false;

	const InstrumentCategoryEligeBTCCredit = '1';
...
class EligeBTC_Payment_Payment_Offer_Details {
...
class EligeBTC_Payment_Payment_Error_Details {
...
class EligeBTC_Payment_Payment_Seller_Details {
	public $eligebtc_account_id          = false;
	...
	public function loadFromDoECResponse( $doECResponse, $bucketNum ) {
		$map = array(
			'SELLERELIGEBTCACCOUNTID'   => 'eligebtc_account_id',
