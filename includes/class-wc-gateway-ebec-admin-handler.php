...
class WC_Gateway_EBEC_Admin_Handler {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_order_action_EBEC_capture_charge', array( $this, 'maybe_capture_charge' ) );

		add_action( 'load-woocommerce_page_wc-settings', array( $this, 'maybe_redirect_to_EBEC_settings' ) );
	...
	public function add_capture_charge_order_action( $actions ) {
		...
		$eligebtc_status  = $old_wc ? get_post_meta( $order_id, '_eligebtc_status', true ) : $order->get_meta( '_eligebtc_status', true );

		// bail if the order wasn't paid for with this gateway
		if ( 'ebec_eligebtc' !== $payment_method || 'pending' !== $eligebtc_status ) {
			return $actions;
		}
		...
		$actions['ebec_capture_charge'] = esc_html__( 'Capture Charge', 'woocommerce-gateway-eligebtc' );
	...
	public function show_decimal_warning() {
		if ( get_option( 'wc_gateway_ppce_display_decimal_msg', false ) ) {
			...
			<div class="updated fade">
				<p>
					<strong><?php _e( 'NOTE: EligeBTC does not accept decimal places for the currency in which you are transacting.  The "Number of Decimals" option in WooCommerce has automatically been set to 0 for you.', 'woocommerce-gateway-eligebtc' ); ?></strong>
				</p>
			</div>
			<?php
	...
	public function filter_checkout_sections( $sections ) {
		...
		$eligebtc_sections = array(
			'wc_gateway_EBEC_with_eligebtc',
		);

		$card_sections = array(
			'wc_gateway_EBEC_with_eligebtc_credit',
		);

		$current_section = isset( $_GET['section'] ) ? $_GET['section'] : '';

		// If the current section is a eligebtc section, remove the card section,
		// otherwise, remove the eligebtc section
		$sections_to_remove = in_array( $current_section, $eligebtc_sections ) ? $card_sections : $eligebtc_sections;
	...
	public function capture_payment( $order_id ) {
		...
		$payment_method = $old_wc ? $order->payment_method : $order->get_payment_method();
		if ( 'ebec_eligebtc' === $payment_method ) {

			$trans_id = get_post_meta( $order_id, '_transaction_id', true );
			$trans_details = wc_gateway_ebec()->client->get_transaction_details( array( 'TRANSACTIONID' => $trans_id ) );
			...
			if ( $trans_id && $this->is_authorized_only( $trans_details ) ) {
				...
				$result = wc_gateway_ebec()->client->do_express_checkout_capture( $params );

				if ( is_wp_error( $result ) ) {
					$order->add_order_note( __( 'Unable to capture charge!', 'woocommerce-gateway-eligebtc' ) . ' ' . $result->get_error_message() );
				} else {
					update_post_meta( $order_id, '_eligebtc_status', ! empty( $trans_details['PAYMENTSTATUS'] ) ? $trans_details['PAYMENTSTATUS'] : 'completed' );

					if ( ! empty( $result['TRANSACTIONID'] ) ) {
						update_post_meta( $order_id, '_transaction_id', $result['TRANSACTIONID'] );
					}

					$order->add_order_note( sprintf( __( 'EligeBTC charge complete (Charge ID: %s)', 'woocommerce-gateway-eligebtc' ), $trans_id ) );
				}
			}
		}
	}
	...
	public function cancel_payment( $order_id ) {
		...
		if ( 'ebec_eligebtc' === $payment_method ) {
			...
			if ( $trans_id && $this->is_authorized_only( $trans_details ) ) {
				...
				$result = wc_gateway_ebec()->client->do_express_checkout_void( $params );

				if ( is_wp_error( $result ) ) {
					$order->add_order_note( __( 'Unable to void charge!', 'woocommerce-gateway-eligebtc' ) . ' ' . $result->get_error_message() );
				} else {
					$order->add_order_note( sprintf( __( 'EligeBTC charge voided (Charge ID: %s)', 'woocommerce-gateway-eligebtc' ), $trans_id ) );
				}
			}
		}
	}
	...
	public function gateway_admin_url( $gateway_class ) {
		_deprecated_function( 'WC_Gateway_EBEC_Admin_Handler::gateway_admin_url', '1.0.4', 'wc_gateway_ebec()->get_admin_setting_link' );

		return wc_gateway_ebec()->get_admin_setting_link();
	}

	/**
	 * Maybe redirect to wc_gateway_EBEC_with_eligebtc from EligeBTC standard
	 * checkout settings.
	 *
	 * @return void
	 */
	public function maybe_redirect_to_EBEC_settings() {
		if ( ! wc_gateway_ebec()->settings->enabled ) {
			return;
		}
		...
		if ( 'checkout' === $_GET['tab'] && 'wc_gateway_eligebtc' === $_GET['section'] ) {
			$redirect = add_query_arg( array( 'section' => 'wc_gateway_EBEC_with_eligebtc' ) );
	...
	public function maybe_reset_api_credentials() {
		...
		//La siguiente linea la he metido para probar. No estaba
		//delete_option( 'woocommerce_EBEC_eligebtc_settings' );

		if ( empty( $_GET['reset_EBEC_api_credentials'] ) ) {
			return;
		}

		if ( empty( $_GET['reset_nonce'] ) || ! wp_verify_nonce( $_GET['reset_nonce'], 'reset_EBEC_api_credentials' ) ) {
			return;
		}
		...
		*/
		...
		wp_safe_redirect( wc_gateway_ebec()->get_admin_setting_link() );
	}
}
