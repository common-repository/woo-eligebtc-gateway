...
class WC_Gateway_EBEC_With_EligeBTC_Addons extends WC_Gateway_EBEC_With_EligeBTC {
	...
	public function scheduled_subscription_payment( $amount, $order ) {
		...
		if ( empty( $billing_agreement_id ) ) {
			wc_gateway_EBEC_log( sprintf( '%s: Could not found billing agreement. Skip reference transaction', __METHOD__ ) );
			return;
		}
	...
	protected function _process_reference_transaction_response( $order, $response ) {
		...
		try {
			if ( ! $client->response_has_success_status( $response ) ) {
				throw new Exception( __( 'EligeBTC API error', 'woocommerce-gateway-eligebtc' ) );
			}
			...
			switch ( $status ) {
				case 'Pending':
					/* translators: placeholder is pending reason from EligeBTC API. */
					$order_note = sprintf( __( 'EligeBTC transaction held: %s', 'woocommerce-gateway-eligebtc' ), $response['PENDINGREASON'] );
				...
				case 'In-Progress':
					...
					$order->add_order_note( sprintf( __( 'EligeBTC payment approved (ID: %s)', 'woocommerce-gateway-eligebtc' ), $transaction_id ) );
					$order->payment_complete( $transaction_id );
					break;
				default:
					throw new Exception( __( 'EligeBTC payment declined', 'woocommerce-gateway-eligebtc' ) );
			}
	...
	public function update_failing_payment_method( $subscription, $renewal_order ) {
		update_post_meta( $subscription->id, '_EBEC_billing_agreement_id', $renewal_order->ebec_billing_agreement_id );
	}
}
