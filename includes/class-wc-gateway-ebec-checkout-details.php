...
class EligeBTC_Checkout_Details {
	...
	public $eligebtc_adjustment               = false;
	...
	public function loadFromGetECResponse( $getECResponse ) {
		...
		$map = array(
			...
			'ELIGEBTCADJUSTMENT'               => 'eligebtc_adjustment',
		...
		if ( $max_wallet_num > -1 ) {
			...
			for ( $i = 0; $i <= $max_wallet_num; $i++ ) {
				$this->wallets[ $i ] = new EligeBTC_Checkout_Wallet_Details();
		...
		if ( $max_payment_num > -1 ) {
			...
			for ( $i = 0; $i <= $max_payment_num; $i++ ) {
				$this->payments[ $i ] = new EligeBTC_Checkout_Payment_Details();
...
class EligeBTC_Checkout_Payment_Details {
	...
	public function loadFromGetECResponse( $getECResponse, $bucketNum ) {
		...
		if ( $max_line_item_num > -1 ) {
			...
			for ( $i = 0; $i <= $max_line_item_num; $i++ ) {
				$items[ $i ] = new EligeBTC_Checkout_Payment_Item_Details();
				$items[ $i ]->loadFromGetECResponse( $getECResponse, $bucketNum, $i );
			}
		}

		$this->shipping_address = new EligeBTC_Address();
...
class EligeBTC_Checkout_Payment_Item_Details {
	public $name = false;
	public $description = false;
	public $amount = false;
	...
	public function loadFromGetECResponse( $getECResponse, $bucketNum, $itemNum ) {
		...
		$this->physical_details = new EligeBTC_Checkout_Payment_Item_Physical_Details();
		if ( ! $this->physical_details->loadFromGetECResponse( $getECResponse, $bucketNum, $itemNum ) ) {
			$this->physical_details = false;
		}

		$this->ebay_item_details = new EligeBTC_Checkout_Payment_Item_Ebay_Item_Details();
...
class EligeBTC_Checkout_Payment_Item_Physical_Details {
	public $weight;
	public $weight_units;
...
class EligeBTC_Checkout_Payment_Item_Ebay_Item_Details {
	public $item_number            = false;
...
class EligeBTC_Checkout_Shipping_Option_Details {
...
class EligeBTC_Checkout_Instrument_Details {
...
class EligeBTC_Checkout_Wallet_Details {
...
class EligeBTC_Checkout_Payer_Details {
	public $phone_number        = false;
	public $email               = false;
	...
	public function loadFromGetECResponse( $getECResponse ) {
		...
		if ( $billing_address_present ) {
			$this->billing_address = new EligeBTC_Address();
...
class EligeBTC_Checkout_Gift_Details {
