...
/**
 * EligeBTC NVP (Name-Value Pair) API client. This client supports both certificate
 * and signature for authentication.
 *
 * @see https://developer.eligebtc.com/docs/classic/api/#ec
 */
class WC_Gateway_EBEC_Client {

	/**
	 * Client credential.
	 *
	 * @var WC_Gateway_EBEC_Client_Credential
	 */
	protected $_credential;

	/**
	 * EligeBTC environment. Either 'sandbox' or 'live'.
	 *
	 * @var string
	 */
	...
	public function __construct( $credential, $environment = 'live' ) {
		...
		if ( is_a( $credential, 'WC_Gateway_EBEC_Client_Credential' ) ) {
	 ...
	 *
	 * @param WC_Gateway_EBEC_Client_Credential $credential Client's credential
	 */
	public function set_credential( WC_Gateway_EBEC_Client_Credential $credential ) {
	...
	public function get_payer_id() {
		$option_key = 'woocommerce_EBEC_payer_id_' . $this->_environment . '_' . md5( $this->_credential->get_username() . ':' . $this->_credential->get_password() );
	...
	/**
	 * Make a remote request to EligeBTC API.
	 *
	 * @see https://developer.eligebtc.com/docs/classic/api/NVPAPIOverview/#creating-an-nvp-request
	 *
	...
	protected function _request( array $params ) {
		try {
			...
			$body = apply_filters( 'woocommerce_eligebtc_express_checkout_request_body', array_merge( $params, $this->_credential->get_request_params() ) );
			$args = array(
				'method'      => 'POST',
				'body'        => $body,
			...
			wc_gateway_EBEC_log( sprintf( '%s: remote request to %s with params: %s', __METHOD__, $this->get_endpoint(), print_r( $body, true ) ) );
			...
			wc_gateway_EBEC_log( sprintf( '%s: returns error: %s', __METHOD__, print_r( $error, true ) ) );
	...
	protected function _validate_request() {
		...
		if ( ! $this->_credential ) {
			throw new Exception( __( 'Missing credential', 'woocommerce-gateway-eligebtc' ), self::INVALID_CREDENTIAL_ERROR );
		}

		if ( ! is_a( $this->_credential, 'WC_Gateway_EBEC_Client_Credential' ) ) {
			throw new Exception( __( 'Invalid credential object', 'woocommerce-gateway-eligebtc' ), self::INVALID_CREDENTIAL_ERROR );
		}

		if ( ! in_array( $this->_environment, array( 'live', 'sandbox' ) ) ) {
			throw new Exception( __( 'Invalid environment', 'woocommerce-gateway-eligebtc' ), self::INVALID_ENVIRONMENT_ERROR );
		}
	}
	...
	protected function _process_response( $response ) {
		if ( is_wp_error( $response ) ) {
			throw new Exception( sprintf( __( 'An error occurred while trying to connect to EligeBTC: %s', 'woocommerce-gateway-eligebtc' ), $response->get_error_message() ), self::REQUEST_ERROR );
		}

		parse_str( wp_remote_retrieve_body( $response ), $result );

		if ( ! array_key_exists( 'ACK', $result ) ) {
			throw new Exception( __( 'Malformed response received from EligeBTC', 'woocommerce-gateway-eligebtc' ), self::REQUEST_ERROR );
		}

		wc_gateway_EBEC_log( sprintf( '%s: acknowleged response body: %s', __METHOD__, print_r( $result, true ) ) );
	 ...
	 *
	 * @see https://developer.eligebtc.com/docs/classic/api/merchant/SetExpressCheckout_API_Operation_NVP/
	 *
	 ...
	 *                                            is needed after returned from EligeBTC.
	 * }
	 *
	...
	public function get_set_express_checkout_params( array $args ) {
		...
		if ( apply_filters( 'woocommerce_eligebtc_express_checkout_allow_guests', true ) ) {
		...
		$params = array_merge(
			...
			array(
				...
				'NOSHIPPING'                    => WC_Gateway_EBEC_Plugin::needs_shipping() ? 0 : 1,
			)
		);
	...
	protected function _get_cancel_url() {
		return add_query_arg( 'woo-eligebtc-cancel', 'true', wc_get_cart_url() );
	}

	/**
	 * Get billing agreement description to be passed to EligeBTC.
	 *
	 * @since 1.2.0
	 *
	...
	protected function _get_billing_agreement_description() {
		...
		$description = sprintf( _x( 'Orders with %s', 'data sent to EligeBTC', 'woocommerce-subscriptions'  ), get_bloginfo( 'name' ) );
	...
	/**
	 * Get details, not params to be passed in EligeBTC API request, from cart contents.
	 *
	...
	protected function _get_details_from_cart() {
		...
		$details = array(
			...
			'items'             => $this->_get_eligebtc_line_items_from_cart(),
		);
	 ...
	 */
	protected function _get_eligebtc_line_items_from_cart() {
		$settings = wc_gateway_ebec()->settings;
		$decimals = $settings->get_number_of_decimal_digits();
	...
	protected function get_details( $details, $discounts, $rounded_total, $total ) {
		...
		// Compare WC totals with what EligeBTC will calculate to see if they match.
		...
		if ( $details['total_item_amount'] != $rounded_total ) {
			if ( 'add' === $settings->get_subtotal_mismatch_behavior() ) {
				...
				// calculations and EligeBTC calculations.
	...
	protected function _get_details_from_order( $order_id ) {
		...
		$details = array(
			...
			'items'             => $this->_get_eligebtc_line_items_from_order( $order ),
		);

		$details = $this->get_details( $details, $order->get_total_discount(), $rounded_total, $order->get_total() );

		// EligeBTC shipping address from order.
		$shipping_address = new EligeBTC_Address;
		...
		if ( ( $old_wc && ( $order->shipping_address_1 || $order->shipping_address_2 ) ) || ( ! $old_wc && $order->has_shipping_address() ) ) {
			...
			// Fallback to billing in case no shipping methods are set. The address returned from EligeBTC
		...
		//
		// @see https://github.com/woothemes/woocommerce-gateway-eligebtc/issues/139
	 ...
	 * @return array Line items
	 */
	protected function _get_eligebtc_line_items_from_order( $order ) {
		$settings = wc_gateway_ebec()->settings;
		$decimals = $settings->get_number_of_decimal_digits();
	 ...
	 *
	 * @see https://developer.eligebtc.com/docs/classic/api/merchant/GetExpressCheckoutDetails_API_Operation_NVP/
	 *
	...
	public function get_do_express_checkout_params( array $args ) {
		...
		$params = array(
			...
			'PAYMENTREQUEST_0_NOTIFYURL'     => WC()->api_request_url( 'WC_Gateway_EBEC' ),
		...
		if ( WC_Gateway_EBEC_Plugin::needs_shipping() && ! empty( $details['shipping_address'] ) ) {
			$params = array_merge(
				$params,
				$details['shipping_address']->getAddressParams( 'PAYMENTREQUEST_0_SHIPTO' )
			);
	...
	/**
	 * Creates a billing agreement with a EligeBTC account holder.
	 *
	 * Used for subscription products in the purchase.
	 *
	 * @see https://developer.eligebtc.com/docs/classic/api/merchant/CreateBillingAgreement_API_Operation_NVP/
	 *
	 * @since 1.2.0
	 *
	 * @param string $token Token from SetExpressCheckout response
	 ...
	 *
	 * @see https://developer.eligebtc.com/docs/classic/api/merchant/BAUpdate_API_Operation_NVP/
	 *
	 * @since 1.2.0
	 *
	 ...
	 *
	 * @see https://developer.eligebtc.com/docs/classic/api/merchant/DoReferenceTransaction_API_Operation_NVP/
	 *
	 * @since 1.2.0
	 *
	 * @param  array $params NVP params
	...
	public function get_do_reference_transaction_params( array $args ) {
		...
		$params = array(
			...
			'NOTIFYURL'     => WC()->api_request_url( 'WC_Gateway_EBEC' ),
	...
	/**
	 * Obtain your Pal ID, which is the EligeBTC–assigned merchant account number,
	 * and other informaton about your account.
	 *
	 * @see https://developer.eligebtc.com/docs/classic/api/merchant/GetPalDetails_API_Operation_NVP/
	 *
	 * @return array NVP response
	 */
	...
	/**
	 * Issues a refund to the EligeBTC account holder associated with a transaction.
	 *
	 * @see https://developer.eligebtc.com/docs/classic/api/merchant/RefundTransaction_API_Operation_NVP/
	 *
	 * @param  array $params NVP params
	 * @return array         NVP response
	 */
	...
	public function test_api_credentials( $credentials, $environment = 'sandbox' ) {
		...
		if ( 'Success' != $result['ACK'] && 'SuccessWithWarning' != $result['ACK'] ) {
			...
			if ( $found_10002 ) {
				...
				throw new EligeBTC_API_Exception( $result );
			}
		}

		update_option( 'woocommerce_EBEC_payer_id_' . $this->_environment . '_' . md5( $this->_credential->get_username() . ':' . $this->_credential->get_password() ), wc_clean( $result['PAL'] ) );
