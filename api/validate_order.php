	...
	if ($tokenS==$tokenR){
		...
		if (in_array($decoded['status'], array('paid', 'confirmed', 'complete')))
		{
          if ($order_status == 'pending' || $order_status == 'wc-pending'){ //Si es 0 es que no esta creada
	    	...
	    	fwrite($myfile, 'El cartId ' .$decoded['cartId'] .' no tiene orden asociada. La validamos.' ."\r\n");

	    	$order_toconfirm = new WC_Order($decoded['cartId']);
			$order_toconfirm->update_status('completed');

	    	fwrite($myfile, 'El cartId ' .$decoded['cartId'] .' se ha creado la orden correctamente.' ."\r\n");
	    	...
	    	fwrite($myfile, 'El cartId ' .$decoded['cartId'] .' no esta en status confirmado por el backoffice de eligebtc' ."\r\n");
	    	header('Content-Type: application/json');
